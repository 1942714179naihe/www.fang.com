...
@section('cnt')
    ...
    <article class="page-container">
        <form class="form form-horizontal" id="form-fang-add" action="{{ route('admin.fang.update',$fang) }}" method="post">
            @csrf
            @method('PUT')
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <span class="select-box" style="width: 362px;">
                        <input type="text" class="select" placeholder="房源面积" name="fang_build_area" value="{{ $fang->fang_build_area }}">
                    </span>
                    <span class="select-box" style="width: 362px;">
                        <input type="text" class="select" placeholder="使用面积" name="fang_using_area" value="{{ $fang->fang_using_area }}">
                    </span>
                </div>
            </div>
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <span class="select-box" style="width: 362px;">
                        <input type="text" class="select" placeholder="建筑年代" name="fang_year" value="{{ $fang->fang_year }}">
                    </span>
                </div>
            </div>
            <div class="row cl">
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <span class="select-box" style="width: 80px;">
                        <input type="text" class="select" placeholder="房源楼层" name="fang_floor" value="{{ $fang->fang_floor }}">
                    </span>
                    <span class="select-box" style="width: 150px;">
                        <select name="fang_shi" class="select">
                            @for($i=1;$i<=6;$i++)
                                <option value="{{ $i }}" @if($fang->fang_shi == $i) selected @endif>{{ $i }}室</option>
                            @endfor
                        </select>
                    </span>
                    <span class="select-box" style="width: 150px;">
                        <select name="fang_ting" class="select">
                            @for($i=1;$i<=6;$i++)
                                <option value="{{ $i }}" @if($fang->fang_ting == $i) selected @endif>{{ $i }}厅</option>
                            @endfor
                        </select>
                    </span>
                    <span class="select-box" style="width: 150px;">
                        <select name="fang_wei" class="select">
                            @for($i=1;$i<=6;$i++)
                                <option value="{{ $i }}" @if($fang->fang_wei == $i) selected @endif>{{ $i }}卫</option>
                            @endfor
                        </select>
                    </span>
                    <span class="select-box" style="width: 150px;">
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9 skin-minimal">
                    ...
                    <div class="radio-box">
                        <label>
                            <input name="is_recommend" type="radio" value="1" @if($fang->is_recommend == '1') checked @endif>
                            推荐
                        </label>
                    </div>
                </div>
...
@section('js')
    ...
    <script>
        ...
        uploader.on('uploadSuccess', function (file, {url}) {
            // 因为是多图片上传，保存到数据表中的图片地址就是一个拼接地址 以#隔开
            let val = $('#pic').val();
            // 拼接成功一个多图片存入数据字段的字符串
            $('#pic').val(val + '#' + url);
            // 多图片上传显示
